...
int CO_command_init(void) {
	...
	addr.sun_path[0]='\0';
	strcpy (addr.sun_path+1, CO_command_socketPath);
	server_len = offsetof(struct sockaddr_un, sun_path) + strlen(addr.sun_path+1);
	
    //if(bind(fdSocket, (struct sockaddr *) &addr, sizeof(struct sockaddr_un)) == -1) {
	if(bind(fdSocket, (struct sockaddr *) &addr, server_len) == -1) {
...
int CO_command_clear(void) {
	...
	if(connect(fd, (struct sockaddr *) &addr, server_len) == -1) {
        return -1;
    }
